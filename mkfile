...
results/%.tsv.coverage:: data/%.tsv
	...
	covbGG1=$(grep GOLDEN_GENE1 $prereq | awk '$6 > 1 {print ;}' | wc -l)
	covbUR1=$(grep UNWANTED_REGION1 $prereq | awk '$6 > 1 {print ;}' | wc -l)
	covbGG2=$(grep GOLDEN_GENE2 $prereq | awk '$6 > 1 {print ;}' | wc -l) 
	covbUR2=$(grep UNWANTED_REGION2 $prereq | awk '$6 > 1 {print ;}' | wc -l)
	covbGG3=$(grep GOLDEN_GENE3 $prereq | awk '$6 > 1 {print ;}' | wc -l)
	#Calculate regl for each genomic region
        reglGG1=$(grep GOLDEN_GENE1 $prereq | wc -l)
        reglUR1=$(grep UNWANTED_REGION1 $prereq | wc -l)
        reglGG2=$(grep GOLDEN_GENE2 $prereq | wc -l)
        reglUR2=$(grep UNWANTED_REGION2 $prereq | wc -l)
        reglGG3=$(grep GOLDEN_GENE3 $prereq | wc -l)
	...
	#Calculate seqb for each genomic region
        seqbGG1=$(grep GOLDEN_GENE1 $prereq | awk '$6 > 1 {sum += $6} END {print sum}')
        seqbUR1=$(grep UNWANTED_REGION1 $prereq | awk '$6 > 1 {sum += $6} END {print sum}')
        seqbGG2=$(grep GOLDEN_GENE2 $prereq | awk '$6 > 1 {sum += $6} END {print sum}')
        seqbUR2=$(grep UNWANTED_REGION2 $prereq | awk '$6 > 1 {sum += $6} END {print sum}')
        seqbGG3=$(grep GOLDEN_GENE3 $prereq | awk '$6 > 1 {sum += $6} END {print sum}')
	...
	#Calculate panel's breadth  of coverage (bcov) for all the loci of interest (Golden Genes)
        ...
        covbGGs=$(grep GOLDEN_GENE $prereq | awk '$6 > 1 {print ;}' | wc -l)
        #Calculate regl for all the loci of interest (Golden Genes)
        reglGGs=$(grep GOLDEN_GENE $prereq | wc -l)
	...
	#Calculate panel's mean depth of coverage (mdcov) for all the loci of interest (Golden Genes)
        ...
        seqbGGs=$(grep GOLDEN_GENE $prereq | awk '$6 > 1 {sum += $6} END {print sum}')
